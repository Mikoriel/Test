...
using UnityEngine;
public class AR_Two : MonoBehaviour
{
    //高レート、中火力、高貫通
    public static float shootTimer;
    public static float shootTimerDeley = 0.07f; //射撃タイマー、射撃間隔

    private Rigidbody2D rb;
    [SerializeField]
    private float moveSpeed = 2.5f, deactiveTimer = 1f; //速度、消えるまでの時間
  
    public static int damageAmount = 1;   //攻撃力
    private bool damage;           //貫通するかしないか
    [SerializeField]
    private bool destroyObj;        //弾を削除するかの判定
    ...
    private void OnTriggerEnter2D(Collider2D collision) //弾が敵、Mapに決まった数当たると消滅するように
    {
